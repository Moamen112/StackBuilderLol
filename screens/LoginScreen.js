...
export default function LoginScreen() {
  ...
  const handleLogin = () => {
    dispatch(resetError());
    dispatch(
      loginThunk({ username: loginForm.username, password: loginForm.password })
    ).then((result) => {
      console.log(result)
    ...
    >
      <SafeAreaView>
        ...
        <View className="gap-y-6 pt-10 ">
          <Text className="text-white text-center text-3xl">Login</Text>
          <LabeledInput
            label="Username"
            name="username"
        ...
        <View className="flex-row justify-center items-center  pt-4">
          <Text className="text-center text-white">Don't have account?</Text>
          <TouchableOpacity onPress={() => navigation.navigate("Registration")}>
            <Text style={{ color: COLORS.textSecondary }}> Signup now.</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    </KeyboardAwareScrollView>
